                                ...
                                <div class="page-title-box">
                                    <div class="page-title-right">
                                        <ol class="breadcrumb m-0">
                                            ...
                                            <a href="{{ route('barangays.create')}}"><button type="button" class="btn btn-success rounded-pill waves-effect waves-light">Add Barangay</button></a>
                            
                            </ol>

                                    </div>
                                    <h4 class="page-title">Barangay</h4>
                                </div>
                            </div>
                        </div>     
                        <!-- end page title --> 
                    ...
                    <tbody>
                        @foreach($barangays as $barangay)
                            <tr>
                                ...
                                <td>{{ $barangay->city->province->name ?? '-' }}</td>
                                <td>{{ $barangay->city->name ?? '-' }}</td>
                                <td>{{ $barangay->name }}</td>
                                <td>{{ $barangay->extra_fee ?? '0.00' }}</td>
                                <td>
                                ...
                                <td>
                                    ...
                                    <form action="{{ route('barangays.destroy', $barangay->id) }}" 
                                        method="POST" 
                                        class="d-inline">
                                        @csrf
                                        @method('DELETE')
